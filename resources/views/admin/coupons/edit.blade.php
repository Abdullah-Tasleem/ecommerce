...
@section('dashboard')
    <div class="container mt-3">
        <h2>Edit Coupon</h2>
        <form action="{{ route('coupons.update', ['code' => $coupon->code]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="discount_type" id="discount_type"
                        ...
                        <option value="fixed"
                            {{ old('discount_type', $coupon->discount_type) === 'fixed' ? 'selected' : '' }}>Fixed</option>
                        <option value="percent"
                            {{ old('discount_type', $coupon->discount_type) === 'percent' ? 'selected' : '' }}>Percentage
                        </option>
                    </select>
                    @error('discount_type')
                        <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="limit_per_user" id="limit_per_user"
                        value="{{ old('limit_per_user', $coupon->limit_per_user) }}"
            ...
            <div class="row g-3">
                <div class="col-md-3 form-check">
                    <input class="form-check-input" type="checkbox" name="first_time_users_only"
                        id="first_time_users_only"
                        {{ old('first_time_users_only', $coupon->first_time_users_only) ? 'checked' : '' }}>
                    <label class="form-check-label" for="first_time_users_only">First-time users only</label>
                </div>

                <div class="col-md-3 form-check">
                    <input class="form-check-input" type="checkbox" name="registered_users_only"
                        id="registered_users_only"
                        {{ old('registered_users_only', $coupon->registered_users_only) ? 'checked' : '' }}>
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    @php
                        $selectedCategories = old('eligible_categories', $coupon->eligible_categories ?? []);
                    @endphp
